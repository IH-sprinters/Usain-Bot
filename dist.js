...
module.exports = (hook) => {
  ...
  hook.datastore.get('commitments', (err, commitments) => {
    ...
    for(const commitment of commitments) {
      ...
      const now = Date.now();
      const created = Sugar.Date(commitment.created);
      if(deadline && deadline <= now && (now - deadline < 86400000) && !('posted' in commitment)) {
        post(`on ${created.medium()}, @${commitment.user_name} committed to ${commitment.message} by ${Sugar.Date(deadline).medium()} - how'd it go?`);
